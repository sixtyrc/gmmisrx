...
- [x] WhatsApp (OpenWA, servidor self-hosted `openwa.dacio.com.ar`, misma sesión que AgendaTurnosEstetica) implementado en `padron/services/openwa.py`, enviado en **background thread** (no bloquea el pipeline si el servidor tarda o está caído) y enganchado best-effort en `notificar_resultado` — **sin probar todavía** porque el servidor OpenWA está caído; con `OPENWA_KEY` vacío el envío simplemente se omite sin romper el pipeline (comportamiento esperado, no error).
- [x] Panel operativo con login (`/padron/`): dashboard con backlog de todas las corridas (fecha, disparado por, **usuario que la ejecutó**, estado, totales, altas/bajas, estado MisRx) y botón "Ejecutar ahora" (con confirmación) para forzar la generación+subida manual. Template de login con el estilo visual de referencia (logo GM Salud + tarjeta centrada).
- [x] Seguridad básica: password mínima de 12 caracteres (`AUTH_PASSWORD_VALIDATORS`), dos roles vía grupos de Django — **Admin** (superuser, acceso total incluido `/admin`) y **Operador** (permiso `padron.can_run_padron` únicamente, solo puede ver el backlog y apretar "Ejecutar ahora", nada más). Auditoría de login (éxito/fallido) vía signals de Django, y de cada ejecución manual, en `AuditLogEntry`. Cada `PadronRun` queda asociado al usuario logueado que lo disparó (`SET_NULL` si el usuario se borra después, no rompe el historial).
...
- [ ] Falta: despliegue real de la app al server (hoy se desarrolla y prueba en local, contra las bases remotas) — incluye correr `crear_usuarios_iniciales` ahí, y setear `ALLOWED_HOSTS` con el dominio/IP real (hoy tiene defaults de desarrollo: `localhost,127.0.0.1,testserver`).
